...
from .utils import strip_comments, mask_literals, flatten_and
...
def _fq_for_column(col: sxe.Column, alias_map: Dict[str,str],
                   view_to_cols: Dict[str,set], col_to_views: Dict[str,set],
...
def canonicalize_predicate(e: sxe.Expression,
    ...
    if isinstance(e, COMPARISONS) and isinstance(e.left, sxe.Column) and isinstance(e.right, sxe.Column):
        l = q(e.left); r = q(e.right)
        if EQUALS and isinstance(e, EQUALS):
            if r < l:
                l, r = r, l
            op = "="
        else:
    ...
    if (isinstance(e, COMPARISONS) or isinstance(e, LIKE_CLASSES)) and isinstance(e.left, sxe.Column):
        ...
        op = "=" if EQUALS and isinstance(e, EQUALS) else e.key.upper()
        return f"{q(e.left)} {op} {ph}"
...
def extract_from_ast(parsed: sxe.Expression, catalog_maps: Dict[str, set],
                     branch_id: int, name_type_rules: List[Tuple[re.Pattern, str]]) -> Dict[str, Any]:
    ...
    # record alias->FQN map for later canonical SQL replacement
    res["alias_map"] = [{"branch": branch_id, "map": alias_map}]

    # JOINs
    join_list = []
    left_ctx = join_scope[0] if join_scope else None
    for j in parsed.find_all(sxe.Join):
        r_label = relation_label(j.this)
        right_name = alias_map.get(r_label, r_label)
        pairs = []
        ...
        if using_cols:
            for c in using_cols.expressions:
                cname = c.name
                can = f"{left_ctx}.{cname} = {right_name}.{cname}"
                pairs.append({
                    "branch": branch_id,
            ...
            v2c = catalog_maps["view_to_cols"]; c2v = catalog_maps["col_to_views"]
            for e in flatten_and(on_expr):
                if EQUALS and isinstance(e, EQUALS) and isinstance(e.left, sxe.Column) and isinstance(e.right, sxe.Column):
                    lc = canonicalize_predicate(e, alias_map, v2c, c2v, join_scope, name_type_rules)
                    pairs.append({
                        "branch": branch_id,
...
# ---------------- Orchestrator ----------------
def extract_query_objects(sql_text: str, catalog_maps: Dict[str, set],
    ...
    def build_branch_sql(b: int) -> str:
        ...
        if from_entries and from_entries[0].get("from"):
            ...
            if isinstance(base_list, list) and base_list:
                from_sql = "FROM " + ", ".join(base_list)
            elif isinstance(base_list, str):
                from_sql = f"FROM {base_list}"
        ...
        # replace any table aliases with fully-qualified names
        alias_entries = _branch_items(extracted.get("alias_map"), b)
        amap = alias_entries[0].get("map", {}) if alias_entries else {}
        for alias, fqn in amap.items():
            if alias != fqn:
                canon = re.sub(rf"\b{re.escape(alias)}\.", f"{fqn}.", canon)
        return f"( {canon} )"
...
class SnowflakeQueryParser:
    ...
    def parse(self, sql_text: str) -> Dict[str, Any]:
        ...
        filters = [w.get("canon") for w in (ex.get("where") or []) if w.get("canon")]
